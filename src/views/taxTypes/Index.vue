    ...
    >
      ...
      >
        <a-form-item label="稅別代碼" name="code">
          <a-input v-model:value="formState.code" placeholder="請輸入稅別代碼" :disabled="!!editingTaxType" />
        </a-form-item>
        <a-form-item label="稅別名稱" name="name">
          <a-input v-model:value="formState.name" placeholder="請輸入稅別名稱" />
        </a-form-item>
        <a-form-item label="稅率 (%)" name="rate">
          <a-input-number v-model:value="formState.rate" :min="0" :max="100" :precision="2" placeholder="5.00" style="width: 150px" />
        </a-form-item>
...
const loadTaxTypes = async (): Promise<void> => {
  loading.value = true
  try {
    const response = await getTaxTypes({ page: (pagination.current || 1) - 1, size: pagination.pageSize }) // Spring Data 分頁從 0 開始
